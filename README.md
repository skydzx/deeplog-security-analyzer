...
```
正常日志:
192.168.1.100 - - [06/Feb/2026:10:00:00 +0800] "GET /api/users HTTP/1.1" 200 1234
192.168.1.100 - - [06/Feb/2026:10:00:01 +0800] "POST /api/login HTTP/1.1" 200 567

异常日志:
192.168.1.100 - - [06/Feb/2026:10:00:02 +0800] "GET /products.php?id=' OR '1'='1 HTTP/1.1" 200 5432
```

**数据预处理流程：**
```
原始日志 → LogParser解析 → 提取日志键 → 构建词汇表 → 滑动窗口 → 训练序列
    ↓           ↓            ↓            ↓           ↓
  文本      正则匹配     提取模板     唯一键映射   window_size
```
...
| 脚本 | 用途 | 条数 |
|-----|------|-----|
| `generate_production_dataset.py` | 生产环境模拟 | 10,900 |
| `generate_apt_dataset.py` | APT高级持续威胁 | 2,000+ |
| `generate_log4j_dataset.py` | Log4j漏洞利用 | 3,000+ |
| `generate_webshell_dataset.py` | WebShell攻击 | 5,000+ |
| `generate_advanced_dataset.py` | 综合攻击场景 | 8,000+ |
